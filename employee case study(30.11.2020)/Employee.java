...
public class Employee {
			...
			while ((line = br.readLine()) != null) // returns a Boolean value
			{
				...
				if (i != 0) {
					if (employee[0].compareTo(del) != 0) {
						...
						// Gross Salary= " + employee[4] + ","+ " Experience= " + employee[5] +"]");
					}
				}
				i++;
			}
	...
	private static void sorting() {
		...
		switch (ab) {
		case 1:
			Collections.sort(list, new EmloyeeId());
			break;
		case 2:
			Collections.sort(list, new EmloyeeName());
			break;
		case 3:
			Collections.sort(list, new EmloyeeDepartment());
			break;
		case 4:
			Collections.sort(list, new EmloyeeManager());
			break;
		case 5:
			Collections.sort(list, new EmloyeeSalary());
			break;
		case 6:
			Collections.sort(list, new EmloyeeExperience());
			break;
		}
		writeDataToCSV();
	...
	public static void employeework() {
		...
		try (BufferedReader br = new BufferedReader(new FileReader("C:\\Users\\Bcp\\Downloads\\Employees (2).csv"));) {
			...
			while (linee != null) {
				...
				// System.out.println("[Employee ID" + attributes[0]+ "," + "Name=" +
				// attributes[1] +", "+ " Department=" + attributes[2] + ","+" Manager=" +
				// attributes[3] +"," +" Gross Salary= " + attributes[4] + ","+ " Experience= "
				// + attributes[5] +"]");
				linee = br.readLine();
				d++;
	...
	public static void writeDataToCSV() {
		...
		try {
			FileWriter fileWriter = new FileWriter("C:\\Users\\Bcp\\\\Downloads\\sorted.csv");
			...
			for (EmployeeDetails employee : list) {
				printWriter.append(employee.getEmployeeid() + DELIMITER + employee.getName() + DELIMITER
						...
						+ employee.getGrosssalary() + DELIMITER + employee.getExperience() + NEW_LINE_SEPARATOR);
			}
			printWriter.flush();
			printWriter.close();
	...
	public static void updateDataToCSV() {
		...
		try {
			FileWriter fileWriter = new FileWriter("C:\\Users\\Bcp\\Downloads\\Employeees.csv");
			BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);
			PrintWriter printWriter = new PrintWriter(bufferedWriter);
			printWriter.append(FILE_HEADER + NEW_LINE_SEPARATOR);
			for (EmployeeDetails employee : list) {
